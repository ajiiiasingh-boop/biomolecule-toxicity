...
There are two ways in. **D1 is easier** and skips GitHub entirely.

### D1 — Double-click, no GitHub

Double-click **`DEPLOY-TO-VERCEL-WINDOWS.bat`** (Windows) or
**`DEPLOY-TO-VERCEL-MAC-LINUX.command`** (macOS/Linux).

A browser opens to log in to Vercel — pick GitHub or email, either is fine and
neither needs a card. Come back to the black window; it uploads this folder,
builds it in the cloud, and prints your live address on a line starting with
`Production:`.

Every later deploy is the same double-click.

The only thing this does not give you is automatic redeployment when you change
a file — for that, use D2.

### D2 — Through GitHub (auto-redeploys on every change)
...
**Do not use the "choose your files" link** — the operating system's file picker
selects files only, so the `api`, `client`, `server` and `shared` folders get
left behind, and without them there is no project. Instead, open the project
folder on your computer, select *everything inside it* (`Ctrl+A` / `Cmd+A`) —
files **and** folders — and **drag** the whole selection onto the dashed drop
area in the browser. Dragging is the only way the web uploader accepts folders. Do **not** drag the outer folder itself — GitHub needs
